...
#     api_url = "https://a1a5iuugp3.execute-api.us-east-1.amazonaws.com/dev/getallgamedetails"
...
def fetch_data_from_api():
    ...
    if response.status_code == 200:
        ...
        if data:
            try:
                items = json.loads(data['body'])  # Parse the 'body' field as a list of dictionaries
                flattened_teams = []
                ...
                for item in items:
                    ...
                    for team in teams:
                        ...
                        team_points = team['teamgamepoints']

                        for member in team['members']:
                            user_id = member['userId']
                            user_name = member['userName']
                            user_email = member['userEmail']
                            user_points = member['usergamepoints']

                            flattened_teams.append({
                                'gameid': game_id,
                                'teamId': team_id,
                                'teamName': team_name,
                                'teamgamepoints': team_points,
                                'userId': user_id,
                                'userName': user_name,
                                'userEmail': user_email,
                                'usergamepoints': user_points
                            })
...
def write_to_csv(data):
    ...
    # fieldnames = ['gameid', 'teamId','teamName', 'teamgamepoints','userId','userName','userEmail','usergamepoints']
    fieldnames = ['gameid', 'teamId', 'teamName', 'teamgamepoints', 'userId', 'userName', 'userEmail', 'usergamepoints']
...
def lambda_handler(event, context):
    try:
        data_from_dynamodb = fetch_data_from_api()
